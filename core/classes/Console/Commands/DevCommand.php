...
class DevCommand extends BaseCommand
{
     ...
     * @var string
     */
    private $signature = "dev {--ignore-env : Ignore to change the APP_ENV in .env file to development} {--ignore-yml : Ignore to change the default database in phinx.yml file to development}";
     ...
     * @var string
     */
    private $description = "Change application environment to development.";
    ...
    public function handle(Input $input, Output $output)
    {
        ...
        try {
            ...
            if ($this->isAppEnvAlreadyDevelopment()) throw new \Exception("Application environment is already ".static::ENVIRONMENT.".", 1);
            ...
            if (!$ignore_env || !$ignore_yml)
            {
                ...
                if (!$ignore_env)
                {
                    if ($this->isAppDbNameAlreadyForDevelopment()) throw new \Exception("Error: The application DB_NAME is already for ".static::ENVIRONMENT.".", 1);
    ...
    public function isAppEnvAlreadyDevelopment()
    {
        $old_env = config("app.env");
        return static::ENVIRONMENT === $old_env;
    }

    public function isAppDbNameAlreadyForDevelopment()
    {
        $phinx_yml = $this->getParsePhinxYml();
