...
class ProdCommand extends BaseCommand
{
     ...
     * @var string
     */
    private $signature = "prod {--ignore-env : Ignore to change the APP_ENV in .env file to production} {--ignore-yml : Ignore to change the default database in phinx.yml file to production}";
     ...
     * @var string
     */
    private $description = "Change application environment to production.";
    ...
    public function handle(Input $input, Output $output)
    {
        ...
        try {
            ...
            if ($this->isAppEnvAlreadyProduction()) throw new \Exception("Application environment is already ".static::ENVIRONMENT.".", 1);
            ...
            if (!$ignore_env || !$ignore_yml)
            {
                ...
                if (!$ignore_env)
                {
                    if ($this->isAppDbNameAlreadyForProduction()) throw new \Exception("Error: The application DB_NAME is already for ".static::ENVIRONMENT.".", 1);
    ...
    public function isAppEnvAlreadyProduction()
    {
        $old_env = config("app.env");
        return static::ENVIRONMENT === $old_env;
    }

    public function isAppDbNameAlreadyForProduction()
    {
        $phinx_yml = $this->getParsePhinxYml();
